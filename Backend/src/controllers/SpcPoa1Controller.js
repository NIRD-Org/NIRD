...
import { SpcPoa1Model } from "../models/SpcPoa1Model.js";
import { Errorhandler } from "../utils/errorHandler.js";
import { uploadFile } from "../utils/uploadFile.js";

const getNewId = async () => {
  try {
    const maxDoc = await SpcPoa1Model.aggregate([
      {
        $addFields: {
          numericId: { $toInt: "$id" },
        },
...
//     let poa1 = await SpcPoa1Model.findOne({
//       user_id,
//       $expr: {
//         $and: [
...
export const getPoa1s = CatchAsyncError(async (req, res, next) => {
  try {
    const poa1Data = await SpcPoa1Model.find({ user_id: req?.user?.id });
...
export const updatePoa1Data = CatchAsyncError(async (req, res, next) => {
  try {
    ...
    let poa1 = await SpcPoa1Model.findOne({ id: poaId });
