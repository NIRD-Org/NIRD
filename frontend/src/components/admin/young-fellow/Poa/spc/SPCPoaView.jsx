...
const SPCPoaView = () => {
  ...
  useEffect(() => {
    const fetchPOARecords = async () => {
      try {
        setLoading(true);
        const response = await API.get("/api/v1/spc-poa1/getUserPOAs");
            ...
            return (
              <TableRow key={idx}>
                ...
                <TableCell className="flex gap-4">
                  <Link to={`/admin/spc/POA1/view/${record.id}`}>
                    <NirdViewIcon />
                  </Link>
                  <Link to={`/admin/spc/POA1/edit/${record.id}`}>
                    <NirdEditIcon />
                  </Link>
                </TableCell>
              </TableRow>
