...
const POA1Form = ({ update }) => {
  const poaType = "poa1"; // Fixed poaType for this form
  ...
  useEffect(() => {
    const fetchFormData = async () => {
      try {
        if (update) {
          const response = await API.get(`/api/v1/pmu-poa/getUserPOAs?poaType=${poaType}`);
  ...
  return (
    <div style={{ fontSize: "14px", maxWidth: "100%", margin: "0 auto" }}>
      <AdminHeader>
        Plan of Action - Week of {months[selectedMonth]} {currentYear}
      </AdminHeader>
            ...
            <TableRow key={day}>
              ...
              <TableCell>
                <input
                  ...
                  onChange={(e) => handleInputChange(day, "target", e.target.value)}
                />
              </TableCell>
              <TableCell>
                <input
      ...
      <div className="flex justify-end">
        <Button pending={loading} onClick={handleSubmit} className="primary-button mt-4">
